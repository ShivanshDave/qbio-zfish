...
class visStim:
    ...
    def __init__(self,speed,size,ori,tex):
        ...
        self.grating = visual.GratingStim(win=self.mywin, size=(100,100), mask=None, pos=[0,0], sf=self.size, ori= self.ori, tex=self.tex, color="red")
    ...
    def sensory_conflic(self, dir=1, N=0, timeout=0):
        def _ftr_(dir1, dir2):
            self.bars_move(dir1,0)
            self.bars_move(dir1,self.speed)
            self.bars_move(dir2,self.speed)
...
#            self.mywin.flip() #flip is preferred to update() (equivalent but deprecated) 
    ...
    def loop_stim(self, s=2, N=0):    
        while True:
            if s==1:
                 self.single_bar_move('+',0,2)
                 self.single_bar_move('-',0,2)
            elif s==2:
                 self.bars_move('+',self.speed, 4)
                 self.bars_move('-',self.speed, 4)
            N-=1
            if N==0:
                 break
...
#s1.sensory_conflic(dir=1, N=4) #--EXP--GroupBehavior
#s1.single_bar_move(N=1, timeout=0) 
s1.loop_stim(N=0)
